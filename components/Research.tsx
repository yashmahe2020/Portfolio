        ...
        <div className={`absolute inset-0 backface-hidden h-full w-full bg-white border border-slate-100 rounded-[2.5rem] p-8 flex shadow-sm group-hover:shadow-xl transition-shadow overflow-hidden ${isHorizontal ? 'flex-row items-center gap-8' : 'flex-col items-center text-center'}`}>
          ...
          <div className={`flex-1 flex flex-col ${isHorizontal ? 'text-left' : 'justify-center text-center'} gap-1`}>
            <div className={`flex items-center gap-2 mb-1 ${isHorizontal ? '' : 'justify-center'}`}>
              ...
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{item.period}</span>
            </div>
          ...
          <div className={`absolute bottom-6 right-8 text-[8px] font-black text-slate-300 uppercase tracking-widest ${isHorizontal ? '' : 'w-full text-center left-0'}`}>
             {isHorizontal ? 'Flip Up' : 'Flip Left'} for Details
          </div>
        </div>

        {/* BACK SIDE */}
        <div className={`absolute inset-0 backface-hidden ${isHorizontal ? 'rotate-x-180' : 'rotate-y-180'} h-full w-full bg-slate-50 border-2 border-slate-100 rounded-[2.5rem] p-6 lg:p-8 flex flex-col items-start shadow-2xl`}>
          <div className="w-full mb-3">
            <div className="flex items-center gap-3 mb-1">
              <div className={`w-6 h-6 rounded-lg ${accentBg} flex items-center justify-center ${accentColor}`}>
                <Info size={12} />
              </div>
              <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">Impact & Results</p>
            </div>
          ...
          <ul className={`flex-1 overflow-y-auto pr-2 custom-scrollbar w-full ${isHorizontal ? 'grid grid-cols-2 gap-x-4 gap-y-2' : 'space-y-2'}`}>
            {item.highlights.map((h, i) => (
              <li key={i} className="flex gap-2 text-[10px] lg:text-[11px] text-slate-600 font-bold leading-tight items-start">
          ...
          <div className="mt-3 pt-2 w-full text-center border-t border-slate-100">
